    ...
    , ui(new Ui::MainWindow)
{
    ...
    ui->SaveFile->setIcon(QPixmap(":/images/Save.jpg"));
    ...
    ui->ExplorerDock->setFeatures(QDockWidget::DockWidgetClosable);
    ui->ExplorerDock->setWindowTitle(tr("Explorer"));
    ui->ListView->setModel(&StrList);
    ui->ActiveFilesDock->setWidget(ui->ListView);
    ui->ActiveFilesDock->setFeatures(QDockWidget::DockWidgetClosable);
...
void MainWindow::LoadFileData(const QString& path) {
       ...
       if (!highlightingRule.isEmpty()) {
          ...
          Highlighter* HL = new Highlighter(codeEditor->document(), highlightingRule);
